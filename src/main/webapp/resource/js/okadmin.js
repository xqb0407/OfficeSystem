...
layui.use(["element", "form", "layer", "okUtils", "okTab", "okLayer", "okContextMenu", "okHoliday", "laydate"], function () {
   ...
   var okTab = layui.okTab({
      ...
      url: "/navs",
      // 允许同时选项卡的个数
      openTabNum: config.openTabNum || 30,
   ...
   $("body").on("click", "#navBar .layui-nav-item a, #userInfo a", function () {
      ...
      if ($(this).siblings().length == 0) {
		if (config.pageTabs) { //开启多页面选项卡功能
			okTab.tabAdd($(this));
		} else { //关闭多页面选项卡功能
			var iframe = $("#tabContent")
				.find(".layui-tab-item")
				.find("iframe");
			var url = $(this).attr("data-url"); //选项卡的页面路径
      ...
      okLayer.confirm("确定要退出吗？", function (index) {
         okTab.removeTabStorage(function (res) {
            okTab.removeTabStorage();
               window.location.href = "/UserServlet?method=logout";
         });
      });
   });
   ...
   function lockShowInit(okUtils) {
      ...
      $("#lockQuit").click(function () {
         ...
         window.location.replace("/UserServlet?method=logout");  //替换当前页面
      });
   }
   //加载控制台
   // $('#ifram').src("/pageServlet?page=Console");
